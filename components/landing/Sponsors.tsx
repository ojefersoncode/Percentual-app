    ...
    <section id="sponsors" className="container text-center pb-12 max-md:pt-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 justify-center">
        {sponsors.map(({ icon, description }, index) => (
          <div
            key={index}
            className="flex pt-10 flex-col items-center justify-between bg-red-600 rounded-lg shadow-lg p-6 hover:shadow-xl transition-all duration-300 min-h-[300px]"
          >
            <div className="flex w-full justify-center items-center">{icon}</div>
            <p className="text-sm text-center text-gray-200 max-md:px-4 pb-10">{description}</p>
          </div>
        ))}
      </div>
    </section>
